...
export class CartService {
  ...
  modifyItem(item: any) {
    return {
      id: item._id,
      title: item.title,
      imageUrl: item.profile.includes(environment.apiUrl)
        ? item.profile
        : environment.apiUrl + '/' + item.profile,
      price: item.price,
      detail: item.detail,
      amount: 0,
  ...
  async postLoginCart() {
    ...
    return await lastValueFrom(this.http.get(`${environment.apiUrl}/carts`))
      .then(async (res: any) => {
        const cartCached: any = JSON.parse(
          localStorage.getItem('CART') ? localStorage.getItem('CART')! : '{}'
        );
        let cartToCache = res.data.filter(
          (item: any) => item.created_user_id == currentUser._id
        );
        let emptyCart = cartToCache.length
          ? cartToCache[0]
        ...
        if ('_id' in cartCached) {
          ...
          formData.append(
            'created_user_id',
            '_id' in currentUser ? currentUser._id : ''
          );
          return await lastValueFrom(
            this.http.put(
              `${environment.apiUrl}/carts/${emptyCart._id}`,
              formData
            )
          )
            .then((res: any) => {
  ...
  async postCreateEmptyCart() {
    ...
    return await lastValueFrom(
      this.http.post(`${environment.apiUrl}/carts`, formData)
    )
      .then((res: any) => {
  ...
  async postUpdateCart() {
    ...
    return await lastValueFrom(
      this.http.put(`${environment.apiUrl}/carts/${cartCached._id}`, formData)
    )
      .then((res: any) => {
        res = res.data;
        localStorage.setItem('CART', JSON.stringify(res));
  ...
  async postDeleteCart() {
    ...
    if (cartCached && '_id' in cartCached) {
      return await lastValueFrom(
        this.http.delete(`${environment.apiUrl}/carts/${cartCached._id}`)
      ).then((res) => {
  ...
  async addToCart(newItem: any) {
    try {
      ...
      console.log(
        'Add to Cart: ' + newItem.title + ' | amount: ' + newItem.amount
      );
      this.cartItem$.next(this.cartItem);
    } catch (e) {
      console.log(e);
