...
export class AuthService {
  ...
  public signUp(payload: any) {
    ...
    formData.append('dob', '');
    return lastValueFrom(
      this.http.post(`${environment.apiUrl}/auth/signup`, formData)
    );
  }
  ...
  postUpdateUserAddress(user: any, address: any) {
    ...
    formData.append(
      'dob',
      `${address.dob.year}-${address.dob.month}-${address.dob.day}`
    );
  ...
  public reset(payload: any): Promise<any> {
    return lastValueFrom(
      this.http.get(`${environment.apiUrl}/forgot-password`, payload)
    );
  }

  public resetPasswordUpdate(
    id: string,
    token: string,
    payload: any
  ): Promise<any> {
    return lastValueFrom(
      this.http.post(
        `${environment.apiUrl}/password-reset-update/${id}/${token}`,
        payload
      )
    );
  }
  ...
  public getUsers() {
    ...
    return lastValueFrom(
      this.http.get(`${environment.apiUrl}/users?page=1&chunk=1000`, options)
    );
  }
  ...
  public deleteUser(userId: any) {
    ...
    return lastValueFrom(
      this.http.delete(`${environment.apiUrl}/users/${userId}`, options)
    );
  }
  ...
  public changePassword(userId: any, resetToken: any, newPassword: any) {
    ...
    return lastValueFrom(
      this.http.post(
        `${environment.apiUrl}/auth/password-reset-update/${userId}/${resetToken}`,
        formData
      )
    );
  }
